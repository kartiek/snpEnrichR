...
'data.frame':	422 obs. of  34 variables:
 ...
 $ CNV                       : chr  "N" ...
```
The data frame contains all SNPs avaible. Hence, it should be parsed to match required criteria e.g. population, p-value etc.

```R
require(rtracklayer)
ch = import.chain('data/hg38ToHg19.over.chain')
GWAS_SNPS_parsed_hg10 <- liftOver(GWAS_SNPS_parsed, ch)
```
